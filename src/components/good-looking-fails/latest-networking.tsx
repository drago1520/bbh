...
import SignUpDialog from '@/app/(frontend)/networking/components/sign-up-modal';
import CountdownTimer from '@/app/(frontend)/networking/components/countdown';
...
export default function LatestNetworking({ isImageRight = false, event, className, ...props }: { isImageRight?: boolean; event: Event | undefined } & ComponentProps<'section'>) {
  ...
  const { speakerName, title, date, maxGuests, thumbnail, description, location, locationImg } = event;
