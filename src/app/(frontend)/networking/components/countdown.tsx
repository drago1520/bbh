...
function UIConferencePricing({ days, hours, minutes, seconds, props }: CountdownRenderProps) {
  const { className } = props;
  const format = (time: number) => time.toString().padStart(2, '0');
  return (
...
export default function CountdownTimer({ endDate, variant = 'default', className }: { endDate: string | number | Date | undefined; className?: string; variant?: 'default' | 'conferencePricing' }) {
  return <Countdown date={endDate} className={className} renderer={variant === 'conferencePricing' ? UIConferencePricing : UI} />;
}
