  ...
  return (
    <Dialog>
      ...
      <DialogContent className="sm:max-w-[425px]">
        <Form {...form}>
          <form
            onSubmit={form.handleSubmit(async (user: z.infer<typeof signUpSchema>) => {
              ...
              <div className="space-y-4">
                <FormField
                  ...
                  render={({ field, fieldState }) => (
                    <FormItem>
                      <FormControl>
                        <FloatingLabelInput error={fieldState.invalid} disabled={isLoading} id="name" label="Име" autoComplete="name" {...field} />
                      </FormControl>
                      <FormMessage />
                    </FormItem>
                  )}
                    ...
                    <FormItem>
                      <FormControl>
                        <div className="flex gap-4">
                          <FloatingLabelInput error={fieldState.invalid} disabled={isLoading} label="Имейл" id="email" type="email" {...field} />
                          <Button className="min-w-24" disabled={isLoading}>
                            {!isLoading ? 'Изпрати' : <LoaderCircle className="animate-spin" />}
                          </Button>
                        </div>
                      </FormControl>
                      <FormMessage />
              ...
              <DialogFooter className="flex-row justify-end gap-6 saturate-80">
                <DialogClose asChild>
                  <Button asChild variant="ghost" size="icon">
                    <Link href="#">
                      <Image src="/google-calendar.svg" alt="Google календар покана" width={40} height={40} />
                    </Link>
                  </Button>
                </DialogClose>
                <DialogClose asChild>
                  <Button asChild variant="ghost" size="icon">
                    <Link href="#">
                      <Image src="/apple-calendar.png" alt="Apple календар покана" width={40} height={40} />
                    </Link>
                  </Button>
                </DialogClose>
              </DialogFooter>
